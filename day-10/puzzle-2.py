...
Unlike in the first part, each set will contain the distinct routes to the peaks from the corresponding location on the map.
For each peak (location where the value of the map is 9) we add its location to the same place in `reachablePeaks`, but this time as the first element of a (currently single-element) tuple.
"""
...
then we take all the distinct routes leading to peaks from the input coordinates, append the location to the end of all these routes, and add them all to the set corresponding to the location.
...
def processNeighbours(topoMap, curPos, curHeight):
    ...
    for i, j in ((0, -1), (0, 1), (-1, 0), (1, 0)):
        if (0 <= curPos[0]+i < mapSize) and (0 <= curPos[1]+j < mapSize) and (topoMap[curPos[0]+i, curPos[1]+j] == curHeight-1):
            [reachablePeaks[curPos[0]+i, curPos[1]+j].add((*reachablePeak, (curPos[0]+i, curPos[1]+j))) for reachablePeak in reachablePeaks[*curPos]]
            neighbours.append((curPos[0]+i, curPos[1]+j))
    return

"""
From 9 to 1 we iterate through all the locations of the map of given height, and process their neighbours. In the end we will have a complete map of all distinct routes leading to peaks from any given position.
"""
...
"""
We iterate through the low points of the map (where the value is 0), and sum the number of distinct routes from each location, which is our result.
"""
result = 0
