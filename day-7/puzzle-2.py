...
"""
We define an `ops` tuple containing the integer addition and the multiplication functions, as well as an anonymous integer concatenation function.
"""
...
First, we check if the starting value is larger than the test value: since none of our operations (addition, multiplication, concatenation) can result in an output less than its inputs, if the starting value is larger than the test value, the test value cannot be constructed, therefore we return false.
...
We optimize by restricting the last operation:
    1. If the test value is not divisible by the last number in the list, we exclude multiplication from the operations in the last place.
    2. If the test value does not end in the last number in the list, we exclude concatenation from the operations in the last place.
...
def testLine(testValue, numList):
    ...
    if not testValue%lastNum:
        lastOp += (ops[1],)
    if str(testValue).endswith(str(lastNum)):
        lastOp += (ops[2],)
